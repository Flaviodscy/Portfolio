              ...
              {experiences.map((exp, i) => (
                ...
                >
                  ...
                  <div className={`ml-12 md:ml-0 md:w-1/2 ${i % 2 === 0 ? "md:pr-16 md:text-right" : "md:pl-16"}`}>
                    ...
                    {exp.location && (
                      <span className="text-xs text-[var(--text-muted)] ml-2">— {exp.location}</span>
                    )}
                    <h4 className="text-xl font-bold mt-1 leading-snug text-[var(--text-primary)]">{exp.role}</h4>
                    <p className="text-emerald-400 font-medium mt-1">{exp.company}</p>
                    <p className="text-[var(--text-secondary)] mt-3 leading-relaxed">{exp.description}</p>
                    {exp.responsibilities && (
                      <ul className={`mt-3 space-y-1 ${i % 2 === 0 ? "md:text-right" : ""}`}>
                        {exp.responsibilities.slice(0, 5).map((r) => (
                          <li key={r} className="text-sm text-[var(--text-muted)] flex items-start gap-2">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {education.map((edu, i) => (
              <div key={i} className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl p-6 card-hover">
                ...
                <h4 className="text-lg font-bold mt-1 leading-snug text-[var(--text-primary)]">{edu.degree}</h4>
                <p className="text-emerald-400 text-sm mt-1">{edu.school}</p>
                <p className="text-[var(--text-muted)] text-sm mt-1">{edu.location}</p>
              </div>
            ))}
          </div>
        </div>
